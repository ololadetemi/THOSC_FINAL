...
const StaffManagement = () => {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      if (newUserId) {
        setStaffPasswords(prev => ({ ...prev, [newUserId]: form.password }));
      }
      setMessage(`✅ Account created for ${form.name}. You can view their password in the table below.`);
    ...
    <Layout>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-8">
        ...
        <p className="text-gray-400 text-xs mb-5">
          Fill in the details below to create a login account for a new staff member.
        </p>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="p-6 border-b border-gray-100">
          ...
          <p className="text-gray-400 text-xs mt-1">
            Passwords are only visible for accounts created in this session
          </p>
        </div>
                ...
                staff.map(s => (
                  <tr key={s._id} className="hover:bg-surface">
                    ...
                    <td className="px-6 py-4">
                      {staffPasswords[s._id] ? (
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-sm text-gray-700">
                            {visiblePasswords[s._id] ? staffPasswords[s._id] : '••••••••'}
                          </span>
                          <button onClick={() => togglePasswordVisibility(s._id)}
                            className="text-gray-400 hover:text-primary-600 transition-colors">
                            <EyeIcon show={visiblePasswords[s._id]} />
                          </button>
                        </div>
                      ) : (
                        ...
                        <div className="flex gap-3">
                          {s.isActive && (
                            <button onClick={() => handleDeactivate(s._id)}
                              className="text-yellow-500 hover:text-yellow-700 text-xs font-medium transition-colors">
                              Deactivate
                            </button>
                          )}
